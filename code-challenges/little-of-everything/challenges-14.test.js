...
let biggerThanLuke = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    ...
    let someoneMass =arr[i].mass
    someoneMass = parseInt(someoneMass,10);
    lukeMass =parseInt(lukeMass,10);
    if(lukeMass < someoneMass){
      biggerPesrons.push(arr[i].name)
    }
  }
  biggerPesrons = biggerPesrons.join('-')
  return biggerPesrons;
};
